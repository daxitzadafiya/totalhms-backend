...
<script src="{{ asset('/vendor/wrappixel/monster-admin/4.2.1/assets/plugins/sweetalert/sweetalert.min.js') }}"></script>
<script src="{{ asset('/filejs/riskareas/checklist/editactivechecklist.js') }}"></script>
<script src="{{ asset('/js/muuri.js') }}"></script>
<script src="{{ asset('/filejs/riskareas/checklist/editmurri.js') }}"></script>
                        ...
                        <div class="col-md-6">
                            <label><b>Name</b></label>
                                <div class="form-group">
                                    <input type="text" name="chklistname" class="form-control" id="naam" required value="{{ (old('chklistname') != '' ) ? old('chklistname') : (isset($chkdata->checklistname) ? $chkdata->checklistname : '') }}">
                                </div>
                        </div>
                            ...
                            <label><b>Kategori</b></label>
                                ...
                                <div class="form-group">
                                    <select name="categ" id="category" class="form-control">
                                        ...
                                        <option {{ isset( $chkdata->category) && $chkdata->category == $cat->id ? "selected" : "" }} value="{{ $cat->id }}">{{ $cat->name }}</option>
                                        @endforeach
                    ...
                    <div class="row">
                        <!-- <div class="col-md-6">
                            <label><b>Type</b></label>
                                <div class="form-group">
                                    <input type="text" name="type" id="type_id" class="form-control" required value="{{ isset($chkdata->types) ? $chkdata->types : '' }}">
                                </div>
                        </div> -->
                        <div class="col-md-6">
                            <label><b>Project</b></label>
                                <div class="form-group">
                                    <input type="text" name="project" id="project_id" class="form-control" required value="{{ isset($chkdata->project) ? $chkdata->project : '' }}">
                                </div>
                        </div>
                    </div>
                    {{ csrf_field() }}
                    <input type="hidden" name="stat_type" value="1">
                    <input type="hidden" name="id" value="{{ $chkdata->id }}">
                        <div class="board ts">
                            @if(count($topic)  > 0 )
                            ...
                            <div class="board-column todo">
                                ...
                                <input type="hidden" name="checklist_id" value="{{ $chkdata->id }}">
                                <div class="board-column-content">
                                @php
                                $qdata = \DB::table("check_question")->where("chktopic_id",$data->id)->get();
                                $i = 1;
                                @endphp
                                @if(count($qdata) != 0)
                                    ...
                                    <div class="board-item">
                                        <div class="board-item-content">
                                            <div class="row ">
                                                    <div class="col-md-10">
                                                        <div class="form-group">
                                                                <input type="text" name="cpoint[{{ $data->id }}][questions][qes][][qid][]" data-qid="{{ $qx->id }}" data-catid="{{ $data->id }}" class="form-control quest" required  value="{{ $qx->question }} " placeholder="Spørsmål.."><small class="invalid-feedback"> Tekst er påkrevd! </small>
                                                                <input type="hidden" name="cpoint[{{ $data->id }}][questions][qid][]" value="{{ $qx->id }}" >
                                                                <small class="invalid-feedback">Tekst påkrevd!</small>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-1">
                                                            <button type="button btn-danger" class="btn btn-danger mobile waves-effect removeItem deleteq">-</button>
                                                    </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input type="hidden" name="checklist_id" id="chklistid" value="{{ isset($chkdata->id) ? $chkdata->id : '' }}">
